...
const paths = [
  '/tests/VMTests/',
  '/tests/VMTests/RandomTests/'
]

const outputPath = '/lintedTests/'
...
function processesPath (filepath) {
  ...
  for (const file of files) {
    processFile(
      ...
      path.join(__dirname, outputPath, filepath.slice(6), file))
  }
}

function processFile (fileLoc, resultLoc) {
  ...
  console.log(fileLoc)
  fs.writeFileSync(resultLoc, JSON.stringify(json, null, 2))
}
